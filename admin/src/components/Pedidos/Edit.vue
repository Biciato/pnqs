		...
		<form @submit.prevent="validate">
			<div class="box">
				...
				<template v-if="subscription.subscription_category_id">
					...
					<common-data :subscription="subscription" @common-data="updateSub($event)"></common-data>
					...
					<br><br>
					<button class="button is-primary" :class="{'is-loading': isLoading}">Salvar</button>
					<a @click="$router.push({ path: '/pedidos-elegibilidade' })" class="button is-info" style="margin-left: 0.5em">Voltar</a>
				</template>
			</div>
		</form>
...
import PraticasModal from './PraticasModal.vue'
...
export default {
	...
	created(){
		...
		subscriptionApi.get(this.$route.params.id).then((res) => {
			...
			this.subscription.economic_activity_start = new Date(res.data.result["economic_activity_start"]),
			this.subscription.implantation_start_dt = new Date(res.data.result["implantation_start_dt"])
	...
	methods: {
        ...
        save() {
            ...
            subscriptionApi.update(this.subscription).then(resp => {
                this.isLoading = false
		...
		updateSub($event) {
			this.subscription = Object.assign({}, this.subscription , $event)
		}
	}
}
